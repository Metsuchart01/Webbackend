...
exports.router.post("/add", async (req, res) => {
    ...
    try {
        ...
        const [saleRows] = await dbConnecDatabase_1.conn.query("SELECT * FROM sales WHERE user_id = ? AND game_id = ?", [userId, gameId]);
...
exports.router.get("/:userId", async (req, res) => {
    ...
    try {
        const [rows] = await dbConnecDatabase_1.conn.query(`SELECT c.gid, g.NameGame, g.price, c.quantity, g.imageGame,
       ...
       WHERE c.id = ?`, [userId]);
        ...
        // ✅ ถ้ามีโค้ดส่วนลด
        if (discountCode) {
            const [codes] = await dbConnecDatabase_1.conn.query("SELECT * FROM DiscountCodes WHERE Code=? AND isActive=1", [discountCode]);
            if (codes.length > 0) {
                const code = codes[0];
...
/* ---------------- ลบเกมออกจากตะกร้า ---------------- */
exports.router.delete("/:userId/:gameId", async (req, res) => {
    const { userId, gameId } = req.params;
    try {
        await dbConnecDatabase_1.conn.query("DELETE FROM Newwcart WHERE id=? AND gid=?", [userId, gameId]);
...
exports.router.post("/checkout/:userId", async (req, res) => {
    const { userId } = req.params;
    const { code, total } = req.body;
    const connection = await dbConnecDatabase_1.conn.getConnection();
    try {
        await connection.beginTransaction();
        // โหลดรายการในตะกร้า
        const [cart] = await connection.query(`SELECT c.gid, g.price, c.quantity
       FROM Newwcart c
       JOIN game g ON c.gid = g.gid
       WHERE c.id = ?`, [userId]);
        if (cart.length === 0) {
        ...
        // ตรวจสอบเงินผู้ใช้
        const [users] = await connection.query("SELECT money FROM users WHERE id = ?", [userId]);
        ...
        for (const item of cart) {
            await connection.query("INSERT INTO sales (user_id, game_id, quantity, purchase_date) VALUES (?, ?, ?, NOW())", [userId, item.gid, item.quantity]);
        }
        // ล้างตะกร้า
...
// ✅ ตรวจสอบโค้ดส่วนลด
exports.router.post("/validate-discount", async (req, res) => {
    ...
    try {
        ...
        await dbConnecDatabase_1.conn.query(`insert into DiscountUsage(cid,UserId) values (?,?)`, [discount.cid, userId]);
        return res.json({
            valid: true,
